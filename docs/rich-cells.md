...
| `aggrid.list( items )` | `{ links = { … } }` | A comma-separated row of values; each item is a string (plain text) or `{ link = target, text? }` (a wikilink). Unparseable links are skipped. |
...
| `aggrid.listColumn( spec )` | `aggridLinkList` | `list()` (or `linkList()`) values |
...
- **Sort, filter, and quick-search use the text, not the markup.** They read each cell's
  ...
  `linkList` / `list` column lists each value separately (a row matches if any of its values is
  selected).
