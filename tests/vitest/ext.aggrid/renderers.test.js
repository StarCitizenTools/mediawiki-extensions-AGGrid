...
describe( 'aggridBadge', () => {
	const badge = () => buildColumnTypes().aggridBadge;

	it( 'renders an object value as a variant-classed pill', () => {
		const el = badge().cellRenderer( { value: { label: 'Active', variant: 'success' } } );
		expect( el.tagName ).toBe( 'SPAN' );
		expect( el.className ).toBe( 'ext-aggrid__badge ext-aggrid__badge--success' );
		expect( el.textContent ).toBe( 'Active' );
	} );

	it( 'resolves variant from a scalar value via variantMap', () => {
		const el = badge().cellRenderer( {
			value: 'Flyable',
			variantMap: { Flyable: 'success', Concept: 'warning' }
		} );
		expect( el.className ).toBe( 'ext-aggrid__badge ext-aggrid__badge--success' );
		expect( el.textContent ).toBe( 'Flyable' );
	} );

	it( 'falls back to defaultVariant then neutral for unmapped scalars', () => {
		expect( badge().cellRenderer( { value: 'X', defaultVariant: 'info' } ).className )
			.toBe( 'ext-aggrid__badge ext-aggrid__badge--info' );
		expect( badge().cellRenderer( { value: 'X' } ).className )
			.toBe( 'ext-aggrid__badge ext-aggrid__badge--neutral' );
	} );

	it( 'rejects an unsafe variant token, falling back to neutral', () => {
		const el = badge().cellRenderer( { value: { label: 'x', variant: 'red"><script>' } } );
		expect( el.className ).toBe( 'ext-aggrid__badge ext-aggrid__badge--neutral' );
	} );

	it( 'rejects an unsafe defaultVariant, falling back to neutral', () => {
		const el = badge().cellRenderer( { value: 'X', defaultVariant: 'bad<script>' } );
		expect( el.className ).toBe( 'ext-aggrid__badge ext-aggrid__badge--neutral' );
	} );

	it( 'derives the label scalar for sort/filter/export', () => {
		expect( badge().valueFormatter( { value: { label: 'Active' } } ) ).toBe( 'Active' );
		expect( badge().valueFormatter( { value: 'Flyable' } ) ).toBe( 'Flyable' );
		expect( badge().comparator( { label: 'b' }, { label: 'a' } ) ).toBeGreaterThan( 0 );
	} );
} );
